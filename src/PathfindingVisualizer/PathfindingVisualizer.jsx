...
// Define initial state of start and finish
...
export default class PathfindingVisualizer extends Component {
	...
	// When pressing mouse button down
	...
	// When entering the new node element
	...
	visualizeDijkstra() {
		this.setState({topMessage: "Creator : Richard"});
	...
	animateDijkstra(visitedNodesInOrder, nodesInShortestPathOrder) {
		for (let i = 1; i <= visitedNodesInOrder.length; i++) {
			// When the last element in vistedNodesInOrder is reached
		...
		for (let i = 1; i < nodesInShortestPathOrder.length - 1; i++) {
			setTimeout(() => {
				...
				if (nodesInShortestPathOrder[i].isWeight) {
					document.getElementById(
						`node-${node.row}-${node.col}`
					).className = "node node-path-weight";
				} else {
					document.getElementById(
						`node-${node.row}-${node.col}`
	...
	render() {
		...
		} else if (topMessage === "Creator : Richard") {
		...
		let textBox = (
			<div className="textBox">
				<div className="weightContainer">
					...
					<button onClick={this.toggleWeight}>
						{changeWeightText}
					</button>
				</div>
